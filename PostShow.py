...
"""PostShow v2 (https://github.com/xbnstudios/show-scripts)
...
def main(args):
    ...
    for row in reader:
        ...
        math_minutes_total = int(time.strftime('%H', time.gmtime(int(float(row[0]))))) * 60 + int(
            time.strftime('%M', time.gmtime(int(float(row[0])))))
        ...
        # LRC
        lrc_output.write('[' + str(math_minutes_total).zfill(2) + ':' + str(math_seconds).zfill(2) + '.' + str(
            math_centiseconds).zfill(2) + ']' + row[2] + '\n')

        # CUE
        if cue_initialized is not 1:
            cue_output.write('REM GENRE Podcast\n')
            cue_output.write(
                'REM COMMENT "This cue file has been generated by PostShow v2 (https://github.com/xbnstudios/show-scripts)"\n')
            cue_output.write('TITLE "' + args.title + '"\n')
            cue_output.write('PERFORMER "' + "..::XANA::.. Creations" + '"\n')
