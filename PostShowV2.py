...
Written by s0ph0s. https://github.com/xbnstudios/show-scripts"""
    ...
    def _save_cue(self, path: str):
        ...
        with open(path, "w") as fp:
            ...
            fp.write(
                ...
                'https://github.com/xbnstudios/show-scripts"\n'
            )
...
class ViewUtil:
    ...
    FOOTER_TEXT = (
        "<Up>/<Down> to move between items; <Enter> "
    ...
    def window_wrap(func):
        def f_wrap(*args, **kwargs):
            footer = urwid.AttrWrap(urwid.Text(ViewUtil.FOOTER_TEXT), "background")
            return urwid.Frame(
                urwid.AttrWrap(func(*args, **kwargs), "background"), footer=footer
            )
...
class EncoderProgress:
    ...
    def __init__(self, controller):
        ...
        self.controller.set_alarm_in(
            EncoderProgress.UPDATE_INTERVAL_SECONDS, self.update_progress
        )
    ...
    def update_progress(self, loop, user_data):
        ...
        self.progressbar.set_completion(self.controller.get_encoder_percent())
    ...
    def get_view(self):
        ...
        self.progressbar = urwid.ProgressBar(
            "progress_background", "progress_foreground"
        )
        controls = [
            divider,
            (
                "pack",
                urwid.Text(
                    EncoderProgress.MSG_TEMPLATE.format(
        ...
        contents = urwid.Padding(urwid.Pile(controls, focus_item=0), left=4, width=40)
        return urwid.Padding(
            urwid.Filler(
                urwid.AttrWrap(urwid.LineBox(contents, "Encoding"), "dialog"), height=8
            ),
            width=50,
            align="center",
        )
...
class TaggerProgress:
    ...
    def get_view(self):
        ...
        return urwid.Padding(
            urwid.Filler(
                urwid.AttrWrap(urwid.LineBox(contents, "Tagging"), "dialog"), height=5
            ),
            width=50,
            align="center",
        )
...
class EnterBasics:
    ...
    def get_view(self) -> urwid.Widget:
        ...
        blank = urwid.Divider()
        buttons = [
            urwid.AttrWrap(urwid.Button("OK", self.handle_ok), "btn", "btn_focus"),
            urwid.AttrWrap(
                urwid.Button("Cancel", self.handle_cancel), "btn", "btn_focus"
            ),
        ]
        controls = [
            ...
            blank,
            urwid.Padding(
                urwid.GridFlow(buttons, 10, 3, 1, "left"),
                left=15,
                right=15,
                min_width=13,
                align="center",
        ...
        contents = urwid.Padding(
            TabbableListBox(urwid.SimpleFocusListWalker(controls)), left=2, width=54
        )
        return urwid.Padding(
            urwid.Filler(
                urwid.AttrWrap(urwid.LineBox(contents, "Basic Metadata"), "dialog"),
                height=10,
            ),
            width=60,
            align="center",
    ...
    def handle_ok(self, button):
        ...
        self.controller.set_metadata(
            EpisodeMetadata(self.number_box.edit_text, self.title_box.edit_text)
        )
...
class ConfirmMetadata:
    ...
    def build_row(label: str, width: int, value: str):
        cols = [
            ...
            (
                ...
                1,
                urwid.AttrWrap(
                    urwid.Edit("", value, multiline=True), "textbox", "textbox_focused"
                ),
            ),
        ]
    ...
    def get_view(self):
        ...
        if self.metadata.composer is not None:
            controls.append(self.build_row("Composer:", 14, self.metadata.composer))
        if self.metadata.accompaniment is not None:
            controls.append(
                self.build_row("Accompaniment:", 14, self.metadata.accompaniment)
            )
        ...
        if self.metadata.lyrics is not None:
            controls.append(self.build_row("Lyrics:", 14, self.metadata.lyrics))
        if self.metadata.comment is not None:
            controls.append(self.build_row("Comment:", 14, self.metadata.comment))
        controls.extend(
            [
                urwid.Divider(),
                urwid.Padding(
                    urwid.GridFlow(buttons, 10, 4, 1, "left"),
                    left=10,
                    right=10,
                    min_width=13,
                    align="center",
        ...
        contents = urwid.Padding(
            TabbableListBox(urwid.SimpleFocusListWalker(controls)), left=5, width=50
        )
        return urwid.Padding(
            urwid.Filler(
                urwid.AttrWrap(urwid.LineBox(contents, "Confirm Metadata"), "dialog"),
                height=20,
            ),
            width=60,
            align="center",
...
class Main:
    ...
    def parse_args() -> argparse.Namespace:
        ...
        parser.add_argument("wav", help="WAV file to convert/use")
        parser.add_argument(
            "outdir",
        ...
        parser.add_argument(
            ...
            "--profile",
            default="default",
            help="the configuration profile on which to base default values",
        )
        parser.add_argument(
            "--no-encode",
            default=False,
            action="store_true",
            help="the MP3 file already exists, don't encode the WAV file.",
        )
        ...
        if not os.path.exists(args.wav):
            errors.append("Source WAV file ({}) does not exist".format(args.wav))
